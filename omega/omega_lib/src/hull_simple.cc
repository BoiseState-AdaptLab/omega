...
Relation SimpleHull(const Relation &R, bool allow_stride_constraint, bool allow_irregular_constraint) {
...
Relation SimpleHull(const std::vector<Relation> &Rs, bool allow_stride_constraint, bool allow_irregular_constraint) {
  ...
  for (int i = 0; i < Rs.size(); i++) {
    ...
    if (Rs[i].n_inp() != num_dim)
      throw std::invalid_argument("relations for hull must have same dimension");
    if (Rs[i].n_out() != 0)
      throw std::invalid_argument("hull calculation must be set relation");
  }
  ...
  if (allow_stride_constraint) {
    std::vector<std::vector<std::pair<EQ_Handle, BoolSet<> > > > strides(l_Rs.size());
    ...
    for (int i = 0; i < strides[0].size(); i++) {
      coef_t c = abs(strides[0][i].first.get_coef(Constr_Vars_Iter(strides[0][i].first, true).curr_var()));
      coef_t old_c = c;
      bool is_stride = true;
      for (int j = 1; j < l_Rs.size(); j++) {
        ...
        for (int k = 0; k < strides[j].size(); k++)
          if (!(strides[0][i].second & strides[j][k].second).empty()) {
            coef_t t = gcd(c, abs(strides[j][k].first.get_coef(Constr_Vars_Iter(strides[j][k].first, true).curr_var())));
            if (t != 1) {
        ...
        for (std::list<coef_t>::iterator k = candidates.begin(); k != candidates.end(); k++) {
          Relation r = Relation::True(num_dim);
          EQ_Handle h = r.and_with_EQ(strides[0][i].first);
          h.update_coef(Constr_Vars_Iter(h, true).curr_var(), -old_c+*k);
          r.simplify();
  ...
  if (allow_irregular_constraint) {
    std::vector<std::vector<std::pair<Variable_ID, std::map<Variable_ID, coef_t> > > >ranges(l_Rs.size());
    for (int i = 0; i < l_Rs.size(); i++) {
      ...
      for (GEQ_Iterator e = l_Rs[i].single_conjunct()->GEQs(); e; e++)
        if ((*e).has_wildcards()) {
          ...
          if (num_wildcard == 1) {
            if (direction) {
              for (std::map<Variable_ID, coef_t>::iterator j = formula.begin(); j != formula.end(); j++)
                j->second = -j->second;
    ...
    for (int i = 0; i < l_Rs.size(); i++) {
      ...
      for (EQ_Iterator e(l_Rs[i].single_conjunct()->EQs()); e; e++)
        if (!(*e).has_wildcards()) {
          ...
          for (Constr_Vars_Iter cvi(*e); cvi; cvi++)
            ...
            case Input_Var: {
              h.update_coef(r.set_var(cvi.curr_var()->get_position()+all_match.size()), cvi.curr_coef());
              break;
            }
            case Global_Var: {
      ...
      for (GEQ_Iterator e(l_Rs[i].single_conjunct()->GEQs()); e; e++) {
        ...
        for (Constr_Vars_Iter cvi(*e); cvi; cvi++)
          ...
          case Wildcard_Var: {
            std::map<Variable_ID, Variable_ID>::iterator p = wc_map.find(cvi.curr_var());
            Variable_ID v;
            if (p == wc_map.end()) {
              v = f_exists->declare();
  ...
  if (l_Rs.size() == 1) {
    ...
    for (int i = 0; i < l_Rs.size(); i++)
      for (Constraint_Iterator e(l_Rs[i].single_conjunct()->constraints()); e; e++)
        for (Constr_Vars_Iter cvi(*e); cvi; cvi++)
    ...
    if (globals.size() > 0) {
      int count = 1;
      for (std::map<Global_Var_ID, int>::iterator i = globals.begin(); i != globals.end(); i++)
        i->second = count++;
      ...
      for (int i = 0; i < l_Rs.size(); i++) {
        ...
        for (EQ_Iterator e(l_Rs[i].single_conjunct()->EQs()); e; e++) {
          ...
          for (Constr_Vars_Iter cvi(*e); cvi; cvi++)
            switch (cvi.curr_var()->kind()) {
            case Input_Var: {
              h.update_coef(r.set_var(cvi.curr_var()->get_position()+globals.size()), cvi.curr_coef());
              break;
            }
            case Global_Var: {
              h.update_coef(r.set_var(globals[cvi.curr_var()->get_global_var()]), cvi.curr_coef());
              break;
            }
            default:
              assert(false);
        ...
        for (Constr_Vars_Iter cvi(*e); cvi; cvi++)
          ...
          case Input_Var: {
            ...
            if (pos > globals_reverse.size())
              h.update_coef(r.set_var(pos-globals_reverse.size()), cvi.curr_coef());
            else {
      ...
      std::vector<std::vector<Relation> > projected(num_dim+1, std::vector<Relation>(l_Rs.size()));
      for (int i = 0; i < l_Rs.size(); i++) {
        projected[num_dim][i] = copy(l_Rs[i]);
        for (int j = num_dim-1; j >= 0; j--) {
          projected[j][i] = Project(copy(projected[j+1][i]), projected[j+1][i].input_var(j+1));
      ...
      for (int i = 0; i < num_dim; i++) {
          ...
          for (GEQ_Iterator e = bound[j].single_conjunct()->GEQs(); e; e++) {
            ...
            if (!skip_lb && coef > 0) {
              ...
              for (int k = 0; k < l_Rs.size(); k++)
                if (k != j &&
                    !Must_Be_Subset(copy(l_Rs[k]), copy(r))) {
        ...
        if (!skip_lb) {  
          for (int j = 0; j < l_Rs.size(); j++) {
              ...
              if (coef > 0) {
                ...
                for (Constr_Vars_Iter cvi(*e); cvi; cvi++)
                  if ((*cvi).var->kind() == Input_Var &&
                      (*cvi).var->get_position()-1 != i) {
                    if (((*cvi).coef > 0 && has_ub[(*cvi).var->get_position()-1]) ||
                        ((*cvi).coef < 0 && has_lb[(*cvi).var->get_position()-1]))
                      s.set((*cvi).var->get_position()-1);
                    else {
                      for (GEQ_Iterator e2 = bound[j].single_conjunct()->GEQs(); e2; e2++)
                        if (e2 != e &&
                            (((*cvi).coef > 0 && (*e2).get_coef((*cvi).var) < 0) ||
                             ((*cvi).coef < 0 && (*e2).get_coef((*cvi).var) > 0))) {
                          s.set((*cvi).var->get_position()-1);
                          break;
                        }
            ...
            if (S.size() > 0) {
              BoolSet<> s(i);
              for (std::set<BoolSet<> >::iterator k = S.begin(); k != S.end(); k++)
                s |= *k;
              for (int k = 0; k < i; k++)
                if (s.get(k)) {
                  BoolSet<> t(i);
              ...
              for (std::set<BoolSet<> >::iterator k = S.begin(); k != S.end(); k++) {
                ...
                for (int kk = 0; kk < i; kk++)
                  if ((*k).get(kk))
                    r = Project(r, r.input_var(kk+1));
                r.simplify(2, 4);
                Relation r2 = Project(copy(r), r.input_var(i+1));
                Relation b = Gist(r, r2, 1);
                b = Approximate(b);
                b = EQs_to_GEQs(b);

                for (GEQ_Iterator e = b.single_conjunct()->GEQs(); e; e++) {
                  coef_t coef = (*e).get_coef(b.input_var(i+1));
                  if (coef > 0) {
                    Relation r = Relation::True(b.n_inp());
                    r.and_with_GEQ(*e);
                    r.simplify();

                    if (Must_Be_Subset(copy(hull), copy(r)))
                      continue;
